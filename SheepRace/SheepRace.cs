...
namespace SheepRace
{
    public partial class SheepRace : Form
    {
        ...
        public void Punters()
        {
            ...
            myPunters[0].MyLabel = lblLove;
            myPunters[0].MyRadioButton = rbLove;
            myPunters[0].MyRadioButton.Text = myPunters[0].Name + " has $" + myPunters[0].Cash;
            myPunters[1].MyLabel = lblDilpreet;
            myPunters[1].MyRadioButton = rbDilpreet;
        ...
        {
            if (myPunters[0].Cash <= 0)//Love
            {
                lblLove.Text = "Love is now broke and cannot continue betting";
                rbLove.Enabled = false;
            }
            if (myPunters[1].Cash <= 0)//Dilpreet
            {
                lblDilpreet.Text = "Dilpreet is now broke and cannot continue betting";
                rbDilpreet.Enabled = false;
            }
        ...
        private void rbLove_CheckedChanged(object sender, EventArgs e)
        {
            //Show that Love is betting in the bettor label
            lblBettor.Text = myPunters[0].Name;
            // Sets the maximum bet based off cash
            udBoxBet.Maximum = myPunters[0].Cash;
        }

        private void rbDilpreet_CheckedChanged(object sender, EventArgs e)
        {
            //Show that Dilpreet is betting in the bettor label
        ...
        private void btnRace_Click(object sender, EventArgs e)
        {
            ...
            if (myPunters[0].Cash < udBoxBet.Value && rbLove.Enabled)
            {
                MessageBox.Show("Sorry but Love does not have enough cash to proceed.");
                timer1.Enabled = false;
            }
            if (myPunters[1].Cash < udBoxBet.Value && rbDilpreet.Enabled)
            {
                MessageBox.Show("Sorry but Dilpreet does not have enough cash to proceed.");
                timer1.Enabled = false;
            }
        ...
        public void btnLockIn_Click(object sender, EventArgs e) // Unlock the Race Button and check bet amount is not more than cash
        {
            try
            {
                ...
                {
                    MessageBox.Show("Love does not have enough to proceed");
                    btnRace.Enabled = false;
                }
                if (myPunters[1].Cash < myPunters[1].myBet.Amount)
                {
                    MessageBox.Show("Dilpreet does not have enough to proceed");
                    btnRace.Enabled = false;
                }
